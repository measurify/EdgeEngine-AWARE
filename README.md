...
evaluates the baselines and the exported PPO policy season by season: on the real trace the
PPO policy trained on the synthetic mixture keeps the same small gap to the rule-based
controller in every season (no brown-outs in winter); on the darker synthetic trace it fails
in winter — the regime the training scenarios do not contain.
