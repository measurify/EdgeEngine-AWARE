...
`cloudy_week`. DQN is clearly below PPO, with a higher variance across seeds, and wastes uplinks
in the fast mode. Adding memory (4-frame
stacking or a recurrent PPO) does not help. Repeating training over 5 seeds
(`examples/train_seeds.py`) gives the mean ± std that a paper should quote.
...
**Data.** The repository ships `data/traces/demo_liguria_2023_hourly.csv`, a clearly
labelled **synthetic** stand-in (generated by `tools/make_demo_trace.py` with models
deliberately different from the simulator's) so that everything runs offline. For real data
run `python tools/fetch_open_meteo.py --lat 44.05 --lon 8.21 --start 2023-01-01 --end 2023-12-31`
(ERA5-Land reanalysis via the Open-Meteo archive, licence CC BY 4.0; only the standard
library is needed). Your own logger data works with the same CSV format
(`data/traces/README.md`). `examples/trace_driven.ipynb` compares the trace statistics with
the synthetic scenarios and evaluates the baselines and the exported PPO policy season by
season.
...
1. **`deployment.PolicyBundle`** — one JSON file that freezes everything the device must
   ...
   `ppo_default.json` (PPO, 2 M steps, network 18→64→64→7).
...
| `examples/train_seeds.py` | train and evaluate one (algorithm, seed) pair: `--algo ppo|dqn|ppo_stack|rppo --seed N --steps ... --out DIR`; writes a JSON with the evaluation rows and learning curve | 20–60 min per run |
...
data/traces/                demo trace (synthetic) and the CSV format description
examples/                   notebooks, their generator scripts, compare_policies.py, train_seeds.py, bundles/
