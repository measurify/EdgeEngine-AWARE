...
The policy input is a `float32` vector of dimension **18**, every component in `[0, 1]`
(`gymnasium.spaces.Box(0, 1, (18,), float32)`). It is produced by
...
| **policy input** | the 18 normalised numbers below | `ObservationBuilder` output | the policy |
...
| 14 | `path_loss_est` | path-loss estimate: from the margin of the last ACK (`PL = P_tx − sensitivity − margin` of the mode used), raised to a mode's link budget when an uplink in that mode is lost | `(PL − 110 dB) / 60 dB`, clipped; 1 if none | radio ACK SNR/RSSI (or LinkCheckAns) + flash link-budget table |
| 15 | `sense_low_cost` | energy of a low-cost sample | `E / E_max`, clipped | hardware profile (flash constant or measured) |
| 16 | `sense_high_cost` | energy of a high-quality sample | `E / E_max`, clipped | hardware profile |
| 17 | `tx_cost` | energy of one uplink attempt in the reference (standard) radio mode | `E / E_max`, clipped | hardware profile |
...
* **The link is observed through its own measurements only.** Component 14 is what a node
  can compute from the SNR of an acknowledgement and its flash link-budget table; the true
  fading state is hidden. Because it is *mode independent*, the policy can compare the
  expected margin of every radio mode from one number.
* **Costs are in the observation** (15–17) even though they are constant within an
...
1. **Hidden environment state.** True soil moisture, the daily clearness index, the intra-day
   cloud process and the slow fading of the radio channel are hidden. The node only has noisy,
   dated samples of the first, a noisy power measurement that mixes the next two, and a
   path-loss estimate that is as old as its last acknowledgement.
...
* **sufficient statistics of the recent past** — the EWMA of harvesting (2), the EWMA of ACK
  outcomes (13), the last link measurement (14), the stored sample with its age and quality (5–7);
...
`k` consecutive observations (frame stacking, `k·18` inputs, trivially done in firmware with a
